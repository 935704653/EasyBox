...
class Spider(Spider):
    ...
    def init(self, extend=""):
        try:
            ...
            # 添加线路排序配置，按">"拆分，去空格并转小写
            play_order_str = ext.get('playOrder', 'Ac>AcJ>QQ>MG>iQ>YK>推荐线路')
            # 按">"拆分，去空格并转小写
            self.play_order = [item.strip().lower() for item in play_order_str.split('>')]
            return None
        except Exception as e:
            # 默认排序
            self.play_order = ['1', '2', '3', '4']
            return None
    ...
    def detailContent(self, ids):
        ...
        # 获取播放线路列表
        vod_play_list = data.get('vod_play_list', [])
        
        # 线路排序逻辑
        def get_play_order(item):
            """根据播放来源名称提取线路编号进行排序"""
            play_from_name = item.get('play_from_name', '')
            play_from_code = item.get('play_from_code', '')
            
            # 尝试从名称中提取数字或字母标识（转小写）
            play_from_name_lower = play_from_name.lower()
            # 匹配数字或字母标识
            match = re.search(r'([a-z0-9]+)', play_from_name_lower)
            if match:
                return match.group(1)
            
            # 尝试从代码中提取
            play_from_code_lower = play_from_code.lower()
            match = re.search(r'([a-z0-9]+)', play_from_code_lower)
            if match:
                return match.group(1)
        ...
        if self.play_order:
            # 创建线路标识到优先级的映射
            order_mapping = {identifier: idx for idx, identifier in enumerate(self.play_order)}
            
            def custom_sort(item):
                play_identifier = get_play_order(item)
                # 如果在配置的排序列表中，使用配置的优先级，否则排在最后
                return order_mapping.get(play_identifier, 999)
            
            # 对播放线路进行排序
            sorted_play_list = sorted(vod_play_list, key=custom_sort)
        else:
            sorted_play_list = vod_play_list
        
        # 处理排序后的线路
        for item in sorted_play_list:
