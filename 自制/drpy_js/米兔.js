globalThis.h_ost = 'https://ky1080.top/';
var key = CryptoJS.enc.Base64.parse("ZTU5ZDQ0YjJlZWYwM2JhMg==");
var iv = CryptoJS.enc.Base64.parse("ZTU5ZDQ0YjJlZWYwM2JhMg==");
...
globalThis.vodids = function (ids) {
    ...
    const data = {
        ...
        vod_remarks: '小虎斑：' + rdata.vod.vod_remarks,
        vod_actor: rdata.vod.vod_actor,
        vod_director: rdata.vod.vod_director,
        vod_content: '小虎斑提醒您：请勿相信任何广告谢谢' + rdata.vod.vod_content,
        vod_play_from: '',
        vod_play_url: ''
    };
    let show
    rdata.vod_play_list.forEach((value) => {
        show = value.player_info.show;
        if (show.includes('GZ')  || show.includes('TX') || show.includes('夸克') || show.includes('蓝光XL') || show.includes('蓝光C') || show.includes('XCloud') || show.includes('蓝光M') || show.includes('蓝光C') || 0) {
...
globalThis.jxx = function (id, url, name, juji) {
    try {
        if (id.includes('xmflv') || url.includes('JPYS')) {
            return {
                parse: 1, url: id + url, jx: 0,
                danmaku: '弹幕库接口' + '&jm=' + name + '&js=' + juji + '&key=gunnimadeshabixiaotou'
            };
        }
        //log(id);
        if (url.includes('m3u8')) {
            return {
                parse: 0, url: url, jx: 1,
                danmaku: '弹幕库接口' + '&jm=' + name + '&js=' + juji + '&key=gunnimadeshabixiaotou'
            };
        }
        if (id.includes('http')) {
        ...
        return {parse: 0, url: '失败了呢滚吧', jx: 0};
    }
}
