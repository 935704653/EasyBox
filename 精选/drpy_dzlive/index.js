...
function naturalSort(arr, key) {
    return arr.sort((a, b) => a[key].localeCompare(b[key], undefined, {
        numeric: true,
        sensitivity: 'base'
    }));
}
...
async function main() {
    let js_order = ['本地', '网盘及彈幕配置', '360影视[官]', '菜狗[官]', '奇珍异兽[官]', '优酷[官]', '腾云驾雾[官]', '百忙无果[官]', '哔哩影视[官]', '哔哩教育[官]', '哔哩大全[官]', '采王[合]', '采王zy[合]', '直播转点播[合]'];
    let js_path = './drpy_js';
    let live_path = './lives';
    ...
    let parse_apis = [
        '巧技2,https://zy.qiaoji8.com/gouzi.php?url=,1',
        '广大侠,https://php.egwang186.top/?url=,0',
        'm3u8TV,https://jx.m3u8.tv/jiexi/?url=,0',
        '虾米,https://jx.xmflv.com/?url=,0',
        '虾米2,https://jx.xmflv.cc/?url=,0',
        '巧技,https://zy.qiaoji8.com/neibu.php?url=,1',
        '晨光,https://www.cgdyw.net/jiexi/jiexi-gf/?url=,0',
        '咸鱼,https://jx.xyflv.cc/?url=,0',
        '邦宁,https://video.isyour.love/player/getplayer?url=,0',
    ...
    channels = channels.concat([{
            "name": "电视家66",
            "type": 0,
            "url": "http://yuhuahx.com/dsj66.txt",
            "epg": "http://epg.112114.xyz/?ch={name}&date={date}"
        },
        {
            "name": "开心2024",
            "type": 0,
            "url": "http://kxrj.site:55/lib/kx2024.txt",
        ...
        },
        {
            "name": "每日更新",
            "type": 0,
            "url": "https://live.zbds.top/tv/iptv6.txt",
            "epg": "http://epg.112114.xyz/?ch={name}&date={date}"
        },
        {
            "name": "APP喵",
            "type": 0,
            "url": "http://175.178.251.183:6689/live.txt",
            "epg": "http://epg.112114.xyz/?ch={name}&date={date}"
        },
        {
            'name': '本地嗅探器直播',
            'url': 'proxy://do=live&type=txt&ext=http://127.0.0.1:5708/ysp',
    ...
    js_files.forEach((it, index) => {
        ...
        if (rname.includes('我的哔哩[官]')) {
            ...
            extras = [
                ...
                '?type=url&params=../json/采集[密]静态.json@采王成人[合]',
            ];
